...
random.addEventListener("click", () => {
    ...
    changeChapter(randomSur)
});

autoplay.addEventListener("click", () => {
    const cont = autoplay.classList.contains("novalid");
    autoplay.classList.remove(cont ? "novalid" : "valid");
    autoplay.classList.add(!cont ? "novalid" : "valid");
    autoplayy(chapter);
...
function autoplayy() {
    ...
    document.querySelector("audio").addEventListener("ended", () => {
        ...
        const rchapter = chapter + 1;
        if (1 <= rchapter && rchapter <= 114){
            changeChapter(rchapter);
            chapter=rchapter
        } else {
            autoplay.classList.remove("valid");
            autoplay.classList.add("valid");
        }
    })
}

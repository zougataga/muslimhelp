...
exports.getSurahAllData = getSurahAllData;
...
function filterAllSalat(allSalatObj, type) {
    let all = [];
    for (const salatObj of allSalatObj) {
        salatObj.timings = filterSalat(salatObj.timings, type);
        all.push(salatObj);
    };
    return all;
}
exports.filterAllSalat = filterAllSalat;

function filterSalat(salatObj, type = "all") {
    if (type === 'all') return salatObj;
    else if (type === 'faculatif') return Object.fromEntries(Object.entries(salatObj).filter(([key, value]) => !key.match(/Fajr|Dhuhr|Asr|Maghrib|Isha/)));
    else if (type === 'obligatoire') return Object.fromEntries(Object.entries(salatObj).filter(([key, value]) => key.match(/Fajr|Dhuhr|Asr|Maghrib|Isha/)));
    else return salatObj
}
exports.filterSalat = filterSalat;

async function getSalat(cityObj, type = "all") {
    const
        ...
        heure = now.getHours(),
        min = now.getMinutes(),
        calendar = filterAllSalat(await getCalendar(lat, lng, month, anne), type),
        a = calendar[0];
    return {
        city,
        type,
    ...
    function getProchaine() {
        let nextPrayer, nextPrayerTime;
        for (const [prayer, time] of Object.entries(a.timings)) {
            const
                prayerHour = parseInt(time.split(':')[0]),
                prayerMinute = parseInt(time.split(':')[1].split(' ')[0]);
            if (prayerHour > heure || (prayerHour === heure && prayerMinute > min)) {
                nextPrayer = prayer;
                nextPrayerTime = time;
                break;
            }
        };
        return { salat: nextPrayer, time: nextPrayerTime }
    }
